function detect_area(folder,filename,xml_dst_path)
%1/4图片200s，压缩完全图片1438s，未压缩图片1487s
LargeSize = 10000;  %检测出区域之后的限制大小
SmallSize = 4;
minp=[];maxp=[];  %用来存储最终的位置的矩阵
all_size = [];

bw = imread([folder filename '.tif']);  %读取原图
bw = im2bw( bw );   % 转二值图像，需要注意是否需要转二值，7 8行二选一。
%bw = im;
...
xml(xml_dst_path,folder,filename,image_size,minp,maxp);